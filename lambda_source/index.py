...
def lambda_handler(event, context):
    ...
    if last_state:
        last_action_time = datetime.strptime(last_state['Timestamp'], '%Y-%m-%dT%H:%M:%S')
    ...
    if state == 'STOPPED' and now - last_action_time > timedelta(days=start_after_days):
        ...
        sns.publish(
            ...
            Message='RDS Cost Manager Instance for instance' + rds_instance_to_manage + 'has been started at ' + now + ' and will be stopped after 30 minutes',
        ...
        schedule_next_event(events, 'start-instance', start_after_days*24*60, function_arn)  # 6 days in minutes
        sns.publish(
            TopicArn=os.environ['SNS_TOPIC_ARN'],
            Message='RDS Cost Manager Instance for instance' + rds_instance_to_manage + 'has been stopped at ' + now + ' and will be started again in 6 days',
